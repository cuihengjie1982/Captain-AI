...
import { Search, Video, FileText, Settings, Zap, MessageSquare, Clock } from 'lucide-react';
...
const MyNotes: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'chat' | 'script' | 'summary'>('all');
  ...
  const filteredNotes = notes.filter(n => 
    (n.content.toLowerCase().includes(searchQuery.toLowerCase()) || n.lessonTitle.toLowerCase().includes(searchQuery.toLowerCase()))
  );
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      <header className="mb-8">
        ...
        <p className="text-slate-500 mt-2">已从您的视频中保存 {notes.length} 条笔记</p>
      </header>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
         ...
         <div className="flex items-center gap-2 self-start md:self-auto">
            ...
            <button onClick={() => setFilter('chat')} className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${filter === 'chat' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>聊天</button>
            <button onClick={() => setFilter('script')} className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${filter === 'script' ? 'bg-purple-100 text-purple-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>文字稿</button>
            <button onClick={() => setFilter('summary')} className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${filter === 'summary' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>要点总结</button>
         </div>
      </div>
               ...
               <div key={note.id} className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-start gap-4">
                     <div className="w-12 h-12 rounded-lg bg-slate-100 flex-shrink-0 overflow-hidden">
                        ...
                        <img src={`https://picsum.photos/seed/${note.lessonTitle}/200/200`} alt="" className="w-full h-full object-cover opacity-80" />
                     </div>
                     <div className="flex-1">
                        <div className="flex justify-between items-start mb-1">
                           <h3 className="font-bold text-slate-900">{note.lessonTitle}</h3>
                           <span className="text-xs text-slate-400">{note.userName}</span>
                        </div>
                        <div className="flex items-center gap-4 text-xs text-slate-500 mb-3">
                           <span className="flex items-center gap-1"><Video size={12} /> {note.timestampDisplay}</span>
                           <span className="flex items-center gap-1"><MessageSquare size={12} /> 1 条笔记</span>
                        </div>
